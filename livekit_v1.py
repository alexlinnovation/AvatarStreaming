...
import asyncio, logging, uuid, time, sys, io, queue, threading
...
class AvatarSession:
    ...
    def _start_video_thread(self):
        if self.video_thread is not None and self.video_thread.is_alive():
            logging.warning("Video thread already running")
            return
    ...
    def _video_processing_worker(self):
        ...
        try:
            while not self.video_thread_stop_event.is_set():
                ...
                if frame_data_list is None or not isinstance(frame_data_list, list) or len(frame_data_list) == 0:
                    logging.warning("Got unexpected data from frame_queue")
                    continue
                jpg_data = frame_data_list[0]
                try:
                    if not isinstance(jpg_data, bytes):
                        logging.error(f"Expected bytes, got {type(jpg_data)}")
                        continue
                    rgba = np.asarray(Image.open(io.BytesIO(jpg_data)).convert("RGBA"), np.uint8)
                ...
                try:
                    ...
                    logging.error(f"rtc.VideoFrame creation error: {e}")
                    continue
                ts = time.perf_counter()
                while not self.video_thread_stop_event.is_set():
                    try:
                        self.video_frame_queue.put((vf, ts), timeout=0.1)
                        break
                    except queue.Full:
                        continue
        except Exception as e:
            logging.error(f"Unexpected error in video processing thread: {e}", exc_info=True)
        finally:
    ...
    async def _video_loop_push(self):
        self.first_frame_ts = None
        while True:
            vf, frame_ts = None, None
            while vf is None:
                try:
                    vf, frame_ts = self.video_frame_queue.get_nowait()
                    self.video_frame_queue.task_done()
                except queue.Empty:
                    await asyncio.sleep(0.001)
                    if self.video_thread_stop_event and self.video_thread_stop_event.is_set():
                        return
                    continue
            if vf is not None and frame_ts is not None:
                try:
                    if self.first_frame_ts is None:
                        self.first_frame_ts = frame_ts
                    ts = frame_ts - self.first_frame_ts
                    await self.av_sync.push(vf, ts)
                except Exception as e:
                    logging.error(f"Error pushing video frame: {e}")
        ...
        async for chunk24, _ in self.kokoro.create_stream(text, voice=voice, speed=1.0, lang="en-us"):
            ...
            while len(buf) >= BUFFER:
                ...
                if self.started_at is None:
                    self.started_at = time.perf_counter()
                ts = time.perf_counter() - self.started_at
                af = rtc.AudioFrame((frame * 32767).astype(np.int16).tobytes(), 16_000, 1, BUFFER)
    ...
    async def stop(self):
        ...
        if self.video_thread and self.video_thread.is_alive():
            ...
            if self.video_thread.is_alive():
                logging.warning("Video thread did not stop gracefully.")
        if self.video_task:
        ...
        if self.sdk:
            try:
                ...
                logging.error(f"Error closing SDK: {e}")
